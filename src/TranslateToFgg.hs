...
errT msg = throwError $ moduleName ++ msg
...
term2Fgg info tm@(TmPair tm1 tm2) ty@(BTyProd ty1 ty2) =
  do varsNs1 <- term2Fgg info tm1 ty1
     ...
     let varsNs = varsNs1 ++ varsNs2
         [internalNs1, internalNs2] = newInternalNodes [ty1, ty2]
         tyNs = newOutNodes [ty]
...
term2Fgg info tm@(TmLInj tm1) ty@(BTySum ty1 ty2) =
  do
    ...
    let internalNs = newInternalNodes [ty1]
        tyNs = newOutNodes [ty]
...
term2Fgg info tm@(TmRInj tm2) ty@(BTySum ty1 ty2) =
  do
    ...
    let internalNs = newInternalNodes [ty2]
        tyNs = newOutNodes [ty]
...
term2Fgg info tapp@(TmIsoApp (IsoAnn iso ti@(ITyBase ty1 ty2)) tm) ty2' | ty2 == ty2' =
  do
    ...
    let tyNs1 = newInternalNodes [ty1]
        tyNs2 = newOutNodes [ty2]
...
iso2Fgg info (IsoVar var) ti@(ITyBase vTy eTy) =
  do
    iso <- except (lookupIso var info)
    _ <- iso2Fgg info iso ti
    let ns = newOutNodes [vTy, eTy]
        e1 = isoEdge ns iso
...
cls2Hgf info (v, e) (vTy, eTy) =
  do
    ...
    let varsNs = varsNs1 ++ varsNs2
        [tyNs1, tyNs2] = newOutNodes [vTy, eTy]
...
{- mkVarNodes tm
   ...
   Make internal nodes from free variables in a term. -}
newInternalNodes :: [BaseType] -> [Node]
newInternalNodes as = [(NnInternal j, TmNodeLabel atp) | (j, atp) <- enumerate as]

newVarNames :: [(String, BaseType)] -> [Node]
newVarNames vtys = [ (NnVar v, TmNodeLabel ty) | (v, ty) <- vtys]

newOutNodes :: [BaseType] -> [Node]
newOutNodes tys = [(NnOut j, TmNodeLabel ty) | (j, ty) <- enumerate tys]
