module Perpl.Util.Indices (PatternedTensor(..)) where

import Perpl.Util.Tensor (Tensor(Scalar), TensorLike(..))
import Perpl.Util.JSON (JSON(JSint, JSarray, JSobject))
import Perpl.Struct.Exprs (Ctor(Ctor))
import Perpl.Struct.Show ()
...
instance TensorLike PatternedTensor where
  weights_to_json f pt = JSobject [("physical", weights_to_json f (physical pt)),
  ...
  tensorId dims = PatternedTensor (Scalar one) dims (axes ++ [ProductAxis axes])
    where axes = zipWith (const PhysicalAxis) dims [0..]

  tensorCtor size (Ctor x as) cs = PatternedTensor (Scalar one) dims (axes ++ [axis])
  ...
  tensorSum tpsizes k = PatternedTensor (Scalar one) [m] [PhysicalAxis 0, axis]
