...
module Perpl.Util.Tensor where
import Perpl.Util.Helpers (enumerate, pickyZipWith)
import Perpl.Util.JSON
import Perpl.Struct.Exprs (Ctor(Ctor), Type)
import Data.List (intercalate)
import Data.Semiring (Semiring, zero, one, times, plus)

class TensorLike tensor where
  -- Creates a JSON object from a weights tensor
  weights_to_json :: Semiring a => (a -> JSON) -> tensor a -> JSON

  fromTensor :: Semiring a => Tensor a -> tensor a

  tensorShape :: Semiring a => tensor a -> [Int]

  -- For [d1, d2, ..., dn], returns a 1-diagonal tensor
  -- with shape [d1, d2, .., dn, (d1*d2*...*dn)]
  tensorId   :: Semiring a => [Int] -> tensor a

  tensorSum  :: Semiring a => [Int] -> Int -> tensor a
  tensorCtor :: Semiring a => (Type -> Int) -> Ctor -> [Ctor] -> tensor a
...
instance TensorLike Tensor where
  weights_to_json f (Scalar n) = f n
  weights_to_json f (Vector ts) = JSarray [weights_to_json f v | v <- ts]
...
unsqueeze _ (Scalar _) = error "unsqueeze index out of range"
...
tensorAdd :: Semiring a => Tensor a -> Tensor a -> Tensor a
tensorAdd ta tb = pure plus <*> ta <*> tb

-- This is not impossible for a semiring, so it's commented out.
-- tensorSub :: Num a => Tensor a -> Tensor a -> Tensor a
-- tensorSub ta tb = pure (-) <*> ta <*> tb

tensorTimes :: Semiring a => Tensor a -> Tensor a -> Tensor a
tensorTimes ta tb = pure times <*> ta <*> tb
...
tensorIdRow :: Semiring n => Int -> Int -> [n]
tensorIdRow i l = replicate i zero ++ [one] ++ replicate (l - i - 1) zero

-- Fills a tensor with a value
constantTensor :: Semiring n => n -> [Int] -> Tensor n
constantTensor = foldr (\ dim t -> Vector (replicate dim t)) . Scalar

-- Tensor filled with zeros
zeros :: Semiring n => [Int] -> Tensor n
zeros = constantTensor zero

-- Tensor filled with ones
ones :: Semiring n => [Int] -> Tensor n
ones = constantTensor one
